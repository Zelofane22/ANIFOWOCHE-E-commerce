...
| US-26 | En tant que client, je demande un retour depuis mon historique de commande ; l'admin approuve/rejette/rembourse | P2 | ✅ Fait (Sprint 5, UI refondue juillet 2026) : demande guidée (motif + commentaire) depuis le détail de commande (`OrderDetail.jsx`), statut du retour affiché |
...
| US-28 | En tant que client connecté, j'ajoute un produit à ma liste de souhaits et je la retrouve dans mon compte | P2 | ✅ Fait (Sprint 5, page dédiée juillet 2026) : `apps/wishlist` + page `/compte/favoris` (`Wishlist.jsx`) |
...
> US-33 (notifications WhatsApp réelles) déplacée en [E15 — Fonctionnalités futures](#e15--fonctionnalités-futures-reportées) : pas d'actualité pour l'instant, mise en place prévue bien plus tard.
...
| US-37 | En tant qu'admin, la base de données est sauvegardée automatiquement et les erreurs serveur sont monitorées (Sentry ou équivalent) | P1 | ✅ Fait (juillet 2026) : SDK Sentry intégré (backend + frontend), workflow de backup quotidien chiffré en place et secrets configurés ([docs/backups.md](backups.md)) |
...
| US-43 | SEO : meta tags dynamiques par produit, sitemap.xml, Open Graph pour le partage | P2 | ⚠️ Partiel (août 2026) : sitemap.xml + robots.txt + meta dynamiques produit côté client (`Seo.jsx`). **Boutiques vendeur** : meta OG dynamiques par boutique servis aux crawlers sociaux via Vercel Routing Middleware (`code/frontend/middleware.js`) + `<Seo>` client dans `PublicShop.jsx`. Limitation : pas de logo de boutique (le modèle `Shop` n'a pas de champ image) → `og:image` = logo par défaut. Backlog : ajouter un champ `logo` sur `Shop` (migration + serializer public + middleware) |
...
Fonctionnalités mises de côté volontairement, sans priorité ni sprint assigné pour l'instant — à
reprendre bien plus tard.

| ID | User Story | Priorité | Constat |
|----|-------------|----------|---------|
| US-33 | En tant que client, je reçois une vraie notification WhatsApp (confirmation, en route) | Reportée | Client WhatsApp Cloud codé mais jamais testé avec un vrai token ; email reste le canal par défaut en attendant — pas d'actualité, mise en place prévue bien plus tard |
