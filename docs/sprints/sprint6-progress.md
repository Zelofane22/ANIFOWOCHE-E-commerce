...
- [ ] US-37 — Sauvegardes automatiques PostgreSQL (Render) + monitoring erreurs (Sentry ou équivalent).
  **Partie code faite (juillet 2026)** : SDK Sentry intégré backend (`settings.py`) et frontend
  (`main.jsx` + error boundary + source maps Vite) avec DSN configurés. Reste : ajouter
  `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT` côté Vercel pour l'upload des source maps
  (voir [ci-cd.md](../ci-cd.md)), activer les backups Render.
