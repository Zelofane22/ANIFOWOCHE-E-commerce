...
**Limites des plans vendeurs** : centralisées dans `apps/sellers/limits.py` (FREE = 50 produits actifs, 5 commandes/mois, hors catalogue principal ; la boutique entreprise, marquée `is_official` sur `Shop`, est exempte). Toute nouvelle règle de quota doit passer par ce module.
